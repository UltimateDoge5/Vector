...
import { HomeIcon, AcademicCapIcon, CheckBadgeIcon, TableCellsIcon, PencilSquareIcon, UsersIcon, Squares2X2Icon } from "@heroicons/react/24/outline";
        ...
        <aside className="w-60 px-5">
            ...
            {role != "student" ? (
                <div className="mt-2 flex flex-col gap-y-1 border-t-2 pt-2">
                    ...
                    />
                    {user?.privateMetadata.role === "admin" ? <SidebarItem name="Klasy" icon={<Squares2X2Icon className="h-5 w-5" />} link="/class" /> : (<></>)}
                </div>
            ) : (
                <></>
            )}
