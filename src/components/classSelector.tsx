...
const urlsToRefresh = ["/grades", "/presence"];
...
export default function ClassSelector({
	...
	const filteredPeople = query === "" ? classes : classes.filter((classObj) => classObj.name.toLowerCase().includes(query.toLowerCase()));

	// Update the cookie
	const updateCookie = (id: number) => {
		document.cookie = `selectedClassId=${id}; path=/;`;
		setSelectedClass(classes.find((c) => c.id === id)!);
		if (urlsToRefresh.includes(url)) window.location.reload();
	};
