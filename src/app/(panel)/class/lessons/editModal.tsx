...
interface Props {
    ...
    teacher: TeacherDto,
    lesson: LessonDto,
    edit: (teacherId: number, lessonId: number) => void
}

export default function EditModal({ open, setOpen, lessons, teachers, teacher, lesson, edit }: Props) {
    ...
    useEffect(() => {
        setSelectedLesson(lessons.find(l => l.id === lesson.id) ?? null);
        setSelectedTeacher(teachers.find(t => t.id === teacher.id) ?? null);
    }, [teacher, lesson, teachers, lessons])
